// components/CourtTimelineDashboard.jsx
...
// ── DESKTOP ────────────────────────────────────────────────
function DesktopTimeline({ courts, events, slots, slotWidth }) {
  const now = new Date();
                  ...
                  {courtEvents.map((event) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {playerNames.length > 0 && (
                          <div className="text-[9px] text-white/90 truncate mt-0.5">
                            👤 {playerNames.join(", ")}
                          </div>
                        )}
                      </div>
...
// ── MOBILE ─────────────────────────────────────────────────
function MobileTimeline({ courts, events }) {
  return (
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between px-5 py-3 bg-gradient-to-r from-emerald-500 to-teal-500">
              ...
              >
                {court.status === "available" ? "Disponibile" : "N/D"}
              </span>
            </div>
...
// ── PRINCIPALE ─────────────────────────────────────────────
export default function CourtTimelineDashboard({ courts, events }) {
  ...
  return (
    <div ref={wrapperRef}>
      {isMobile ? (
        <MobileTimeline courts={courts} events={events} />
      ) : (
        <DesktopTimeline
          courts={courts}
          events={events}
